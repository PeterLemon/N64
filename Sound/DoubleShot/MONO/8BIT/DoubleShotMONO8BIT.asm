; N64 'Bare Metal' Sound Double Shot Mono 8BIT Demo by krom (Peter Lemon):
...
Start:
  ...
  li t1,7 ; Load Sample Bit Rate (Bitrate-1)
  sw t1,AI_BITRATE(t0) ; Store Sample Bit Rate To AI Bit Rate Reg $A4500014

  li t1,(VI_NTSC_CLOCK/(44100/4))-1 ; Load Sample Frequency: (VI_NTSC_CLOCK(48681812) / FREQ(44100 / 4)) - 1
  sw t1,AI_DACRATE(t0) ; Store Sample Frequency To AI DAC Rate Reg $A4500010
  li t1,123144 ; Length Of Sample Buffer
  sw t1,AI_LEN(t0) ; Store Length Of Sample Buffer To AI Length Reg $A4500004

AIBusy:
  lb t1,AI_STATUS(t0) ; Load AI Status Reg Byte $A450000C
  andi t1,t1,$40 ; And AI Status With AI Status DMA Busy Bit ($40XXXXXX)
  bnez t1,AIBusy ; If True AI DMA Is Busy
  nop ; Delay Slot

  li t1,(VI_NTSC_CLOCK/(22050/4))-1 ; Load Sample Frequency: (VI_NTSC_CLOCK(48681812) / FREQ(22050 / 4)) - 1
  sw t1,AI_DACRATE(t0) ; Store Sample Frequency To AI DAC Rate Reg $A4500010
  li t1,123144 ; Length Of Sample Buffer
  sw t1,AI_LEN(t0) ; Store Length Of Sample Buffer To AI Length Reg $A4500004
