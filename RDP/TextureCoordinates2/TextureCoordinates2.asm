// Tests texture wrap around for texture coordinates, including subpixel accuracy (Subpixel precision when doing textured rectangles)
// This reproduces the black lines issue in e.g. Mario Kart's character selection screen
...
arch n64.rdp
  ...
  macro Rectangle(variable left, variable top, variable height, variable dtdy, variable t_5) {
    variable shift_s(0)
    variable mask_s(3)
    variable clamp_s(0)
    variable mirror_s(0)
    ...
    // blendMask=0, alphaDither=0, rgbDither=3, combKey=0, convertOne=0, biLerp0=1, biLerp1=1, texFilter=2, tlutType=RGBA16, tlutEnabled=off, lod=off, sharpen=off, detail=off, textPersp=off, cycleType=1 cycle, colorDither=off, pipeline=on
    Set_Other_Modes FORCE_BLEND|CYCLE_TYPE_1_CYCLE|BI_LERP_0|BI_LERP_1|SAMPLE_TYPE|RGB_DITHER_SEL_NO_DITHER|ATOMIC_PRIM
    Sync_Pipe
    Sync_Load
    Sync_Tile
    ...
    Texture_Rectangle right,bottom, 0, left,top, 0<<5,t_5, 1<<10, dtdy
  }
  ...
  while columnIndex < 4 {
    ...
    while rowIndex < 4 {
      // t = 0
      Rectangle(((10 + columnIndex * 16) << 2), ((10 + rowIndex * 12) << 2) + columnIndex, (8 << 2) + rowIndex, 1<<10, 0)

      // t = 8 (quarter of a pixel)
      Rectangle(((10 + columnIndex * 16) << 2), ((70 + rowIndex * 12) << 2) + columnIndex, (8 << 2) + rowIndex, 1<<10, 8)

      // t = 16 (half of a pixel)
      Rectangle(((10 + columnIndex * 16) << 2), ((130 + rowIndex * 12) << 2) + columnIndex, (8 << 2) + rowIndex, 1<<10, 16)
  ...
  while columnIndex < 8 {
    variable rowIndex(0)
    while rowIndex < 8 {
      Rectangle(((100 + columnIndex * 16) << 2), ((10 + rowIndex * 20) << 2) + columnIndex, (8 << 3) + rowIndex, 1<<9, 0)
      variable rowIndex(rowIndex + 1)
    }
...
Texture8x8x16b:
  ...
  dh $F801,$F801,$F801,$F801,$F801,$F801,$F801,$F801
  dh $F801,$F801,$F801,$F801,$F801,$F801,$F801,$F801
  dh $39CF,$39CF,$39CF,$39CF,$39CF,$39CF,$39CF,$39CF
